...
## Framework
The Figure illustrates the overall architecture of the Hypergraph-Enhanced Adaptive Spatio-Temporal Convolutional Recurrent Network (ASTHCRN). Initially, a projection layer maps raw data into a high-dimensional latent space to learn richer feature representations. These features are then processed by the core of the network, which consists of stacked spatio-temporal blocks with residual connections, centered on the Adaptive Hypergraph Convolutional Recurrent Unit (AHGCRU). At each time step, this unit first employs the Adaptive Hypergraph Construction Module (AdaHCM) to dynamically generate a hypergraph that captures time-varying, high-order relationships among spatial entities. Subsequently, the Hypergraph Convolutional Recurrent Unit (HGCRU) leverages this dynamic structure to aggregate information, extracting complex spatio-temporal features. Finally, the output from the spatio-temporal blocks is fed into a prediction layer, where two 2D convolutional layers transform these features to generate forecasts for each entity over the specified horizon.
